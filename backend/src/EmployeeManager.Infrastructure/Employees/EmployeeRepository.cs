...
public sealed class EmployeeRepository : IEmployeeRepository
{
    ...
    public async Task<List<Employee>> ListByManagerAsync(Guid managerId, int skip, int take, CancellationToken ct)
    {
        var entities = await _db.Employees
            .Include(e => e.Phones)
            .Where(e => e.IsActive && e.ManagerEmployeeId == managerId)
            .OrderBy(e => e.FirstName)
            .Skip(skip)
            .Take(take)
            .ToListAsync(ct);
    ...
    public async Task<List<Employee>> SearchByManagerAsync(Guid managerId, string query, int take, CancellationToken ct)
    {
        var q = query.Trim().ToLower();
        if (string.IsNullOrWhiteSpace(q)) return new List<Employee>();
    ...
    public Task<bool> IsManagedByAsync(Guid employeeId, Guid managerId, CancellationToken ct)
        => _db.Employees.AnyAsync(e =>
            e.Id == employeeId &&
            e.IsActive &&
            e.ManagerEmployeeId == managerId, ct);
